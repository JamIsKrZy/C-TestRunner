...
use crate::{collect::{CollectErr, FileCollection}, configs::{Config, TargetConfig}};
...
fn config_init(){
    ...
    CONFIG_VARS.set({
        let setting_toml_str =  fs::read_to_string(SOURCE_CONFIG)
            .expect("Config file not found!");

        let setting = toml::from_str::<TargetConfig>(&setting_toml_str)
            .expect("Unable to parse config!");

        println!("{:?}", setting);
     
        let target_toml_str = fs::read_to_string(setting.setting.config_path.to_owned() + "/config.toml").expect("Unable to locate config file!");
        let mut config = toml::from_str::<Config>(&target_toml_str).expect("Unale to parse target config!"); 
...
fn main() {
    ...
    let file_collection = locate_bin_files()
        .unwrap_or_else(|e| {
...
mod configs{
    ...
    impl Default for TargetPath{
        fn default() -> Self {
            Self { config_path: Default::default(), bin_target: Default::default() }
        }
    }
